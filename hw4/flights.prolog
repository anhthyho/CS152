...
route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  ...
  \+member(Arriving, Visited). %does not go to somewhere already visited

route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
  flight(Departing, Mid, DepartureTime, MidArrival, InitCost), %new edge, dep->mid
  \+member(Mid, Visited),
  route(Mid, Arriving, [Mid | [Departing | Visited]], MidTime, ArrivalTime, RestList, RestCost),
  MidArrival @< MidTime,
  TotalCost is InitCost + RestCost,
  Flights = [Departing | RestList].
