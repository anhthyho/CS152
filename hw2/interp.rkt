...
(define (eval-if c thn els env)
  (cond
    [(eq? (car(evaluate c env)) #t) (cons(car(evaluate thn env)) env)]
...
(define (eval-while c body env)
  (cond [(eq? (car(evaluate c env)) #t) (eval-while c body (cdr (evaluate body env)))]
        [else (cons 0 env)]))
...
(define (eval-assign var exp env)
  (let* ([new (car(evaluate exp env))])
    (cons new (hash-set env var new))))
    

;; Handles sequences of statements
(define (eval-seq e1 e2 env)
  (let* ([first (evaluate e1 env)]
         [v1 (car first)]
         [env1 (cdr first)]
         
         [second (evaluate e2 env1)]
         [v2 (car second)]
         [env2 (cdr second)])
    (cons v1 env2)))
