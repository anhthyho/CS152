  ...
  // ***YOUR CODE HERE***
   return function(){
    for (let i = 0; i < preList.length; i++) {
      ...
      if (!cond) {
        throw new Error("Contract violation in position " + i + ". Expected " + preList[i].expected + " but received " + cond.expected + ".  Blame -> Top-level code");
      }
      let res = f.apply(this, arguments);
      if (!post(res)) {
        throw new Error("Contract violation. Expected " + post.expected + " but returned " + res + "d. Blame -> " + f.name);
      }
      return res;
    }
  }
